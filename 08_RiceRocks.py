...
def group_collide(a_group, other_object):
    for element in set(a_group):
...
def group_group_collide(group_a, group_b):
    collisions = 0
    for element in set(group_a):
        if group_collide(group_b, element):
            collisions += 1
            #print "missile hit"
            group_a.remove(element)
    return collisions
...
def rock_spawner():
    ...
    rock_vel = [(random.random() * .6 - .3) * (.01 * score + .5), (random.random() * .6 - .3) * (.01 * score + .5)]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if not (a_rock.collide(my_ship)):
        rock_group.add(a_rock)
    else:
        #print "ignored because on ship!"
        return    
# initialize stuff
